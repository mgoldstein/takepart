    ...
    <div class="vote" <?=wa('finalists')?>>
       ...
       $finalists = wl('finalists', true);
        ...
        <? foreach ( $finalists as $i => $w ): ?>
            ...
            <?endif?>
                <div class='finalist'>
                    <a href="<?=wu('intelchange_interim_vote')?>#<?=$w->token?>" class="<?=$i==0?'active':''?> <?= (!w($w->token . '_vote_form')->form->vote_allowed && w($w->token . '_vote_form')->form->last_vote == w($w->token . '_vote_form')->form->vote_token) ? 'voted' : '' ?>">
                        <div class="portrait">
                            <div class="img-wrap">
                                <img src="<?=$w->img_src?>" alt="<?=$w->single(false)?>">
                            </div>
            ...
            <div id='<?=$w->token?>' class="finalist<?=$i==0?' active':''?>" data-finalistToken="<?=$w->token?>">
                ...
                <div class="details">
                    ...
                    <p class="vote-btn important">
                        <a href="<?=wu('intelchange_interim_vote')?>#vote_<?=$w->token?>"><?=wr(w('vote_finalist_button'), $w)?></a>
                    </p>
                    ...
                    <div class="modal-wrapper">
                        ...
                        <div class="modal vote-register">
                            ...
                            <div class='fb-connect-wrapper'>
                                <fb:login-button onlogin="takepart_facebookapi_check_login_session0()" registration-url="/user/register?fbreg=true&destination=<?=urlencode(wu('intelchange_interim_vote'))?>" fb_register="true" fb_only="false" width="530">Log In</fb:login-button>
                            </div>
                        </div>
                        <? else: ?>
                        ...
                        <div class="modal vote-thanks">
                            ...
                            <div class="addThis thanks-share">
                                <a class="addthis_button_facebook at300b" title="Facebook" href="#"
                                    addthis:url="<?=wu('intelchange_interim_vote')?>?finalist=<?=$w->token?>#<?=$w->token?>">
                                ...
                                <a class="addthis_button_twitter at300b" title="Tweet" href="#"
                                    addthis:url="<?=wu('intelchange_interim_vote')?>#<?=$w->token?>"
