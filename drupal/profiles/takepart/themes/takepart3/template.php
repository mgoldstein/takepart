...
function takepart3_preprocess_views_view(&$vars) {
  if (isset($vars['view']->name)) {
    $function = 'takepart3_preprocess_views_view__'.$vars['view']->name;
...
//For style reasons, we want to nest the subheadings under the title:
function takepart3_preprocess_views_view__promo(&$vars) {
  if($vars['display_id'] == 'block_1') {
    $fields = $vars['view']->style_plugin->rendered_fields;
    for ($i=0;$i<count($fields);$i++) {
      //pick some subheadings:
      if(array_key_exists('field_article_subhead', $fields[$i])) {
        $subheading = $fields[$i]['field_article_subhead'];
        unset($vars['view']->style_plugin->rendered_fields[$i]['field_article_subhead']);
      }
      if(array_key_exists('title', $fields[$i])) {
        $vars['view']->style_plugin->rendered_fields[$i]['field_article_subhead'] = $fields[$i]['field_article_subhead'] . '<div>' . $subheading . '</div>';
       // print $vars['view']->style_plugin->rendered_fields[$i]['field_article_subhead'];
       // print "<br/>";
      }
    }
  }
...
function takepart3_preprocess_views_view__video(&$vars) {
  ...
  if($vars['display_id'] == 'block_4') {
    ...
    //$vars['raw_result'] = 
    $videos = $vars['view']->style_plugin->rendered_fields;
    ...
    for ($i=0;$i<count($videos);$i++) {
      ...
      //$ifid = strip_tags($fields['field_video_embedded']->content);
      //$imgpath = file_load($ifid)->uri;
      //print file_create_url($imgpath);
      
      $fid = ($node->field_video_embedded['und'][0]['fid']);
      $file = file_load($fid);
      print_r($node);
      exit();
