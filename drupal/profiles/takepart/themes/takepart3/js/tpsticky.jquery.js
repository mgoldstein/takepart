	...
	return this.each(function() {
		...
		var check_scroll = function() {
			if ( $window.scrollTop() + $this.outerHeight() > $offset.offset().top + $offset.height() ) {
				$this.removeClass('above').removeClass('inside').addClass('below');
			} else if ( $window.scrollTop() > $offset.offset().top ) {
				$this.removeClass('above').removeClass('below').addClass('inside');
				$this.css({marginTop: $window.scrollTop() - $offset.offset().top, left: left});
			} else {
				$this.removeClass('below').removeClass('inside').addClass('above');
		...
		$window
			...
			.bind('resize', function() {
				$this.removeClass('below').removeClass('inside').removeClass('above');
