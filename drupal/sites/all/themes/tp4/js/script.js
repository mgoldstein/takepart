...
//     $('#block-menu-menu-megamenu ul li a').not('li.is-leaf a').doubleTapToGo();
	 ...
	 function makeTall() {
	   $(this).find('.mega-content').fadeIn(100);
	 }
	 function makeShort() {
	   $(this).find('.mega-content').fadeOut(100);
	 }
  ...
  Drupal.behaviors.featuredCampaignsModule = {
    attach: function () {
	 ...
	 var calculateWidth = function () {
	   ...
	   if (getTranslateX() > lastScrollPoint)
		scrollTo(lastScrollPoint);
	   calculateNav();
	 };
	 ...
	 hammerTime.on('drag', function (e) {
	   if (['up', 'down'].indexOf(e.gesture.direction) > -1)
		return;
	   e.preventDefault();
